...
- **Lineage** — a pan/zoom graph of generation chains, with a resizable viewer pane on the right. Click a node to inspect that link: the **prompt** and the **generated media** are separate. Chain names in the left list use a short unique label from Gemini Embedding 2 when the Archive title is a filename, UUID, or `image (1)`; readable prompts such as “write a sentence about love” stay. Right-click any node → **Use as Basis** adds that node **and its ancestor media** to Creation Studio so the next CREATE branches from that node (an image edit stays an image edit, not a mixed-source report). Full CREATE prompts are stored in `lineage.sqlite` (next to `archives.json`). **Show orphans** is off by default so imports and older items with no recorded prompt do not fill the list. In-place extract / Apply keep one node and stamp `revisedAt`. **Open in Viewer** on a media node still opens the full Viewer tools.
...
| **Lineage** | Connected generation trees (hidden orphans by default). Parent media → prompt node → child. Click a node to view that prompt or that result in the resizable pane on the right (they stay separate). Left-list names are short unique labels from the file content (embeddings) when the Archive title is junk. Right-click **Use as Basis** adds that node and its ancestor media to Studio; the next CREATE branches from it. **Show orphans** lists imports and older items with no recorded prompt. **Show in Lineage** from Archives/Viewer still opens that item even if it is an orphan. |
